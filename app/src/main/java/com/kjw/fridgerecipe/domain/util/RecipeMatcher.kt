...
class RecipeMatcher @Inject constructor(
     ...
     */
    private fun toStandard(word: String, synonyms: Map<String, String>): String {
        return synonyms[word] ?: word
    }

    suspend fun calculateMatch(recipe: Recipe, fridgeIngredients: List<Ingredient>): RecipeWithMatch = coroutineScope {
        // 최신 동의어 사전 로드
        val synonyms = getRemoteSynonyms()

        // 1. 냉장고 재료 병렬 분석 (성능 최적화)
        val fridgeDataDeferred = fridgeIngredients.map { ingredient ->
        ...
        val missingIngredients = recipeIngredientsToCheck.filterNot { recipeIng ->
