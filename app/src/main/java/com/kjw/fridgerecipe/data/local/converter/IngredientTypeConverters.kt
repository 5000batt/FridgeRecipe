...
class IngredientTypeConverters {
    ...
    fun toUnitType(value: String?): UnitType? {
        return value?.let { 
            try { UnitType.valueOf(it) } catch (e: Exception) { UnitType.COUNT }
        }
    }
    ...
    fun toStorageType(value: String?): StorageType? {
        return value?.let { 
            try { StorageType.valueOf(it) } catch (e: Exception) { StorageType.REFRIGERATED }
        }
    }
    ...
    fun toIngredientCategoryType(value: String?): IngredientCategoryType? {
        return value?.let { 
            try { IngredientCategoryType.valueOf(it) } catch (e: Exception) { IngredientCategoryType.VEGETABLE }
        }
    }
    ...
    fun toIngredientIcon(value: String?): IngredientIcon? {
        return value?.let { 
            try { IngredientIcon.valueOf(it) } catch (e: Exception) { IngredientIcon.DEFAULT }
        }
    }
}
