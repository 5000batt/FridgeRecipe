    ...
    fun validate(state: RecipeEditUiState): ValidationResult {
        return when {
            ...
            state.ingredientsState.isEmpty() -> 
                ValidationResult.Failure(RecipeValidationField.INGREDIENTS, UiText.StringResource(R.string.error_recipe_ingredients_empty), ListErrorType.IS_EMPTY)
            
            state.ingredientsState.any { it.name.isBlank() || it.quantity.isBlank() } -> 
                ValidationResult.Failure(RecipeValidationField.INGREDIENTS, UiText.StringResource(R.string.error_recipe_ingredients_blank), ListErrorType.HAS_BLANK_ITEMS)
            
            state.stepsState.isEmpty() -> 
                ValidationResult.Failure(RecipeValidationField.STEPS, UiText.StringResource(R.string.error_recipe_steps_empty), ListErrorType.IS_EMPTY)
            
            state.stepsState.any { it.description.isBlank() } -> 
                ValidationResult.Failure(RecipeValidationField.STEPS, UiText.StringResource(R.string.error_recipe_steps_blank), ListErrorType.HAS_BLANK_ITEMS)
