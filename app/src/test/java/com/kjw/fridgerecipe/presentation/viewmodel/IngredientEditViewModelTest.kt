...
class IngredientEditViewModelTest {
    ...
    @Test
    fun `수정 저장 성공 시 ShowSnackbar SUCCESS 후 NavigateBack 순서로 emit`() =
        runTest {
    ...
    @Test
    fun `이름 비어있음 오류 시 ScrollToField NAME emit`() =
        runTest {
    ...
    @Test
    fun `수량 오류 시 ScrollToField AMOUNT emit`() =
        runTest {
    ...
    @Test
    fun `삭제 성공 시 ShowSnackbar SUCCESS 후 NavigateBack 순서로 emit`() =
        runTest {
    ...
    @Test
    fun `삭제 DB 실패 시 ShowSnackbar ERROR emit`() =
        runTest {
            coEvery { getIngredientByIdUseCase(any()) } returns DataResult.Success(fakeIngredient)
